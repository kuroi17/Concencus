...
function ChannelSidebar({ collapsed = false, onCollapseChange, onCloseMobile }) {
        ...
        <nav className="flex flex-1 flex-col items-center gap-4">
          {loadingChannels ? (
            ...
            categories.map((category) => {
              ...
              const hasActiveChild = category.channels.some((ch) => currentChannel?.id === ch.id);
              return (
  ...
  return (
    <>
            ...
            <div className="space-y-6 px-1">
              {[...Array(4)].map((_, i) => (
                <div key={i} className="space-y-3">
                  ...
                  {[...Array(2)].map((__, j) => (
                    <div key={j} className="h-9 w-full animate-pulse rounded-xl bg-slate-100/60 dark:bg-slate-800/60" />
                  ))}
                </div>
              ))}
            </div>
                  ...
                  {isOpen && (
                    <ul className="space-y-1" role="list">
                            ...
                            >
                              ...
                              {isAdmin && (
                                ...
                                >
                                  <span
                                    role="button"
                                    onClick={() => setChannelModal({ mode: "edit", channel: ch })}
                      ...
                      {isAdmin && (
                        <li>
                          <button
                            onClick={() => setChannelModal({ mode: "create", category: category.id })}
                            className="flex w-full items-center gap-2 rounded-xl px-3.5 py-2 text-xs font-semibold text-slate-400 hover:text-teal-500 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
                          >
                            <Plus size={13} />
                            Add channel
                          </button>
      ...
      {channelModal && (
        <ChannelFormModal
          initial={channelModal.mode === "edit" ? channelModal.channel : null}
          fixedCategory={channelModal.mode === "create" ? channelModal.category : null}
          categories={categories}
          onSave={async (data) => {
            if (channelModal.mode === "create") {
              await createChannel({ ...data, category: channelModal.category });
            } else {
